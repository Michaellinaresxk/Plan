      ...
      <div className='space-y-4'>
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            min='1'
            max={serviceData?.metaData?.maxPeople || 100}
            className={`w-full p-2 border rounded-md ${
          ...
          {serviceData?.metaData?.maxPeople && (
            <p className='text-xs text-gray-500 mt-1'>
              {t('serviceForm.maxGuests', {
                max: serviceData.metaData.maxPeople,
              })}
            </p>
          )}
        </div>
        ...
        {Object.keys(serviceOptions).length > 0 && (
          <div className='border rounded-md p-4 bg-gray-50'>
            <h4 className='font-medium text-gray-800 mb-3'>
              {t('serviceForm.options')}
            </h4>

            {Object.entries(serviceOptions).map(([category, option]) => {
              ...
              return (
                <div key={category} className='mb-4 last:mb-0'>
                  <label className='block text-sm font-medium text-gray-700 mb-2'>
                    {option.nameKey ? t(option.nameKey) : category} *
                  </label>

                  <select
                    value={formData.selectedOptions[category] || ''}
                    onChange={(e) =>
                      handleOptionChange(category, e.target.value)
                    }
                    className={`w-full p-2 border rounded-md ${
                      errors[category] ? 'border-red-500' : 'border-gray-300'
                    }`}
                  >
                    <option value=''>{t('serviceForm.selectOption')}</option>
                    ...
                    {Object.entries(option.subOptions).map(
                      ([subOptionKey, subOption]) => {
                        ...
                        const price = subOption.price || 0;
                        const priceDisplay =
                          price === 'double'
                            ? ` (${t('serviceForm.doubles')})`
                            : price !== 0
                            ? price > 0
                              ? ` (+$${price})`
                              : ` (-$${Math.abs(price)})`
                        ...
                        return (
                          <option key={subOptionKey} value={subOptionKey}>
                            {subOption.nameKey
                              ? `${t(subOption.nameKey)}${priceDisplay}`
                              : `${subOptionKey}${priceDisplay}`}
                          </option>
                        );
                      }
                    )}
                  ...
                  {formData.selectedOptions[category] &&
                    option.subOptions[formData.selectedOptions[category]]
                      ?.descriptionKey && (
