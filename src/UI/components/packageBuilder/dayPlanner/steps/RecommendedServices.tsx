...
const smartCategorizeServices = (
  ...
  const availableServices = services.filter(
    (service) => !currentDaySelectedIds.has(service.id)
  );
...
export const RecommendedServices: React.FC<RecommendedServicesProps> = ({
      ...
      const matchesCategory =
        ...
        (selectedCategory === 'water' &&
          [
            ...
            'boat',
            'swim',
          ].some((term) => service.id.includes(term))) ||
        (selectedCategory === 'relax' &&
          ['massage', 'spa', 'yoga', 'meditation', 'relax'].some((term) =>
        ...
        (selectedCategory === 'romantic' &&
          ['couple', 'romantic', 'private', 'sunset', 'wine'].some((term) =>
            service.id.includes(term)
          ));
  ...
  const handleServiceSelect = (service: Service) => {
    ...
    if (available.length > 0) {
      ...
      alert(
        'No hay horarios disponibles para este día. Por favor, seleccione otro día o elimine alguna actividad existente.'
      );
    }
  };
